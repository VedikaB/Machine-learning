...
X_phoneme_1_temp = np.zeros((np.sum(phoneme_id==1), 2))
# X_phoneme = ...
position=np.zeros(np.sum(phoneme_id==1))
j=0
for i in range(len(phoneme_id)):
    if(phoneme_id[i]==1):
        position[j]=i
        j=j+1
f1_temp=np.zeros(np.sum(phoneme_id==1))
f2_temp=np.zeros(np.sum(phoneme_id==1))
a=0
...
for k in range(len(f2)):
    for h in range(len(position)):
        if k==position[h]:
            f2_temp[b]=f2[k]
            b=b+1
X_phoneme_1_temp[:,0]=f1_temp
X_phoneme_1_temp[:,1]=f2_temp
print(X_phoneme_1_temp)
########################################
...
plot_data(X=X_phoneme_1_temp, title_string=title_string, ax=ax2)
