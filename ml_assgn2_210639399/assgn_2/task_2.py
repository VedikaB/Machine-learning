...
# X_phoneme = ...
#X_phoneme = np.array([])
position=np.zeros(np.sum(phoneme_id==p_id))
j=0
for i in range(len(phoneme_id)):
    if(phoneme_id[i]==p_id):
        position[j]=i
        j=j+1
f1_temp=np.zeros(np.sum(phoneme_id==p_id))
f2_temp=np.zeros(np.sum(phoneme_id==p_id))
a=0
for z in range(len(f1)):
