...
for i in range(len(f1)):
    if phoneme_id[i] == 1:
        if np.size(X_phonemes_1_2) == 0:
            ...
            rowAdd = [X_full[i][0], X_full[i][1]]
            X_phonemes_1_2 = np.vstack((X_phonemes_1_2, rowAdd))
for i in range(len(f1)):
    if phoneme_id[i] == 2:
        if np.size(X_phonemes_1_2) == 0:
...
gridi=np.arange(min_f1,max_f1)
gridj=np.arange(min_f2,max_f2)
for i in range(N_f2):
    print(i)
    for j in range(N_f1):
        grid=np.array([[gridi[j],gridj[i]]])
